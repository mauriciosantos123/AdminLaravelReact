...
<div class="card">
    <div class="card-body p-4">
        ...
        <div class="form-body mt-4">
            <form action="{{route('manutenceService.update',$reg->id)}}" method="POST" enctype="multipart/form-data">
                {{ csrf_field() }}
                @method('PUT')
                <div class="row">
                    <div class="col-lg-8">
                        <div class="border border-3 p-4 rounded">
                            ...
                            <div class="col-12 ">
                                ...
                                <select class="single-select" data-placeholder="Escolha um" name="client_id">
                                    ...
                                    <option value="{{$list->id}}" {{$reg->client_id == $list->id   ? 'selected' : ''}}>00{{$list->id}}.{{ $list->name_fantasy }} </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3 mt-2">
                                <label for="type_service" class="form-label">Descrição*</label>
                                <textarea required readonly class="form-control" id="type_service" name="type_service" rows="4" maxlength="255" placeholder="EX:Falha ao enviar o cadastro para o servidor">{{$reg->type_service}}</textarea>
                            </div>
                            ...
                            <div class="col-12 ">
                                ...
                                <select class="form-select" name="service_id"  required>
                                    ...
                                    <option value="{{$list->id}}" {{$reg->service_id == $list->id ? 'selected' :''}}>{{ $list->name }} </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <select name="categorypayments_id" id="extra" class="form-select"  onchange="showDiv('hidden_div', this)">
                                        ...
                                        <option value="{{$list->id}}" {{$reg->categorypayments_id == $list->id ? 'selected' :''}}>{{ $list->name }} </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control money " id="extra_value" name="extra_value" readonly value="{{$reg->extra_value}}" placeholder="R$ 00.00">
                                </div>
                            </div>
                        ...
                        <div class="border border-3 p-4 rounded">
                            <div class="row g-3">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input readonly value="{{$reg->dt_start}}" type="date" class="form-control" id="dt_start" name="dt_start" min="1950-01-01" max="2050-01-01" maxlength="8" required>

                                </div>
                                <div class="col-md-12">
                                    <label for="completion_time" class="form-label mt-2">Previsão de Termino</label>
                                    <input type="date" value="{{$reg->completion_time}}" class="form-control" id="completion_time" name="completion_time" min="{{$reg->dt_start}}" max="2050-01-01" maxlength="8">
                                </div>
                                <div class="col-md-12">
                                    <label for="dt_finish" class="form-label">Termino do Serviço</label>
                                    <input type="date" value="{{$reg->dt_finish}}" class="form-control" id="dt_finish" name="dt_finish" min="{{$reg->dt_start}}" max="2050-01-01" maxlength="8">
                                </div>
